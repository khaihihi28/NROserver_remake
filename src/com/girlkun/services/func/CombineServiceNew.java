...
public class CombineServiceNew {
    ...
    public static final int NANG_CAP_BONG_TAI3 = 517;
    public static final int MO_CHI_SO_BONG_TAI3 = 518;
    ...
    public void showInfoCombine(Player player, int[] index) {
        ...
        switch (player.combineNew.typeCombine) {
            ...
            case NANG_CAP_BONG_TAI3:
                if (player.combineNew.itemsCombine.size() == 3) {
                    Item bongTai = null, manhVo = null;
                    for (Item item : player.combineNew.itemsCombine) {
                        if (item.template.id == 921) {// id bông tai
                            bongTai = item;
                        } else if (item.template.id == 933) { // id nguyên liệu
                            manhVo = item;
                        }
                    }
                    if (bongTai != null && manhVo != null && manhVo.quantity >= 99) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Nội dung thông tin viết ở đây(item cần nâng cấp số lượng nguyên cần nâng cấp)", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Nội dung thông tin viết ở đây(item cần nâng cấp số lượng nguyên cần nâng cấp)", "Đóng");
            ...
            case MO_CHI_SO_BONG_TAI3:
                if (player.combineNew.itemsCombine.size() == 3) {
                    ...
                    for (Item item : player.combineNew.itemsCombine) {
                        if (item.template.id == 1149) {//id bông tai
                            bongTai = item;
                        } else if (item.template.id == 934) {//id nguyên liệu
                            manhHon = item;
                        } else if (item.template.id == 935) {
                            daXanhLam = item;
                        }
    ...
    private void nangCapBongTai3(Player player) {
        if (player.combineNew.itemsCombine.size() == 2) {
            ...
            if (bongTai != null && manhVo != null && manhVo.quantity >= 99) {
                ...
                InventoryServiceNew.gI().subQuantityItemsBag(player, manhVo, 99);
                if (Util.isTrue(player.combineNew.ratioCombine, 100)) {
                    bongTai.template = ItemService.gI().getTemplate(1149);
                    bongTai.itemOptions.add(new Item.ItemOption(72, 2));
                    sendEffectSuccessCombine(player);
                } else {
                    sendEffectFailCombine(player);
            ...
            if (linhthu != null && thangtinhthach != null && thangtinhthach.quantity >= 99) {
                ...
                if (Util.isTrue(player.combineNew.ratioCombine, 100)) {
                    ...
                    if (rdUp == 0) {
                        ...
                        linhthu.itemOptions.add(new Item.ItemOption(94, Util.nextInt(5, 20)));
                    } else if (rdUp == 5) {
...
//                        }
    ...
    private String getTextTopTabCombine(int type) {
        switch (type) {
            ...
            case NANG_CAP_BONG_TAI3:
                return "Ta sẽ phù phép\ncho bông tai Porata cấp 2 của ngươi\nthành bông tai hắc ám";
            case MO_CHI_SO_BONG_TAI3:
                return "Ta sẽ phù phép\ncho bông tai hắc ám của ngươi\ncó 1 chỉ số ngẫu nhiên";
            case NANG_CAP_LINH_THU:
