                        ...
                        this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                "Cố Gắng Có Làm Mới Có Ăn Con, đừng lo lắng cho ta.\n"
                                        .replaceAll("%1", player.gender == ConstPlayer.TRAI_DAT ? "Quy lão Kamê"
                                                : player.gender == ConstPlayer.NAMEC ? "Trưởng lão Guru" : "Vua Vegeta") + "\b|7|Ta đang giữ tiền tiết kiệm của con\n \b|10|hiện tại con đang có: " + player.getSession().hello + " thỏi vàng\b|4|hiện tại con đang có: " + player.getSession().dangu + " Đá Ngũ Sắc\b|2|hiện tại con đang có: " + player.getSession().nro3s + " ngọc rồng 3 sao ",
                ...
                if (canOpenNpc(player)) {
                    if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                        if (player.getSession().is_gift_box) {
                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, đang x2 rút vàng, khô máu đi con", "Sự kiện\nTết", "Giải tán bang hội", "Nhận quà\nđền bù");
                        } else {
                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con đang khuyến mãi x2, tiền trong tài khoản con là: " + player.session.vnd + "Coin", "Top Sức Mạnh", "Giải tán bang hội", "Rút vàng từ coin", "Đổi thỏi vàng sang ruby");
                        }
                    }
                }
            }
...
//                                        
                                ...
                                break;
                            case 2:
                                NpcService.gI().createMenuConMeo(player, ConstNpc.RUT_VANG, -1, "Con đang có: " + player.session.vnd + " coin trong tài khoản, chọn số lượng rút:",
                                                "20000 40 Thỏi Vàng", "50000 120 Thỏi Vàng", "100000 280 Thỏi Vàng","500000 2000 Thỏi Vàng" ,"Huỷ");
                                break;
                            case 3:
     ...
     private static Npc thienbinn(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (this.mapId == 173) {
                        this.createOtherMenu(player, ConstNpc.BASE_MENU, "Đa vũ trụ song song \b|7|Con muốn gọi Quái Vật để Cày Ngọc Rồng Không \b|1|Với giá 100tr vàng không?", "Gọi Quai Vat\nNhân bản", "Từ chối");
                    }
                }
            }
                            ...
                            switch (select) {
                                case 0: {
                                    ...
                                    if (oldBossClone != null) {
                                        ...
                                        try {
                                            new NguoiChoi(Util.createIdBossClone((int) player.id), bossDataClone, player.zone);
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (mapId == 174 || mapId == 5) {
                        this.createOtherMenu(player, 0, "Map Fam \nHỗ trợ cho Ae Từ\b|1|Dưới 20tỉ SM dến 160 Tỷ SM?", "OK", "Oéo");
                    }
                    if (mapId == 123) {
                    ...
                    if (mapId == 122) {
                        this.createOtherMenu(player, 0, "Thiện Sờ taii\b|7|Thí chủ đang có: " + player.NguHanhSonPoint + " điểm ngũ hành sơn\b|1|Thí chủ muốn đổi cải trang x4 chưởng ko?", "Âu kê", "Top Ngu Hanh Son", "No");
                    }
                }
            }
       ...
       private static Npc gokuac(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (this.mapId == 175) {
                        this.createOtherMenu(player, ConstNpc.BASE_MENU, "Shinn\b|2|Đa vũ trụ song song \b|7|Bro Có Muốn Gọi Quái Vật Fam Được Nhiều Thứ Không \b|1|Với giá một villa không?", "|1|Gọi Quái Vật\nFarm VPSK","|2|Gọi Quái Vật\nFam Đồ", "ĐOÉO");
                    }
                }
            }
                            ...
                            switch (select) {
                                case 0: {
                                    ...
                                    if (oldBossClone != null) {
                                        ...
                                        try {
                                            new FamVang(Util.createIdBossClone((int) player.id), bossDataClone, player.zone);
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
                                        //trừ vàng khi gọi boss
...
//                                      
                                        ...
                                        Service.getInstance().sendThongBao(player, "Không đủ điểm bạn còn " + (2000 - player.pointPvp) + " Điểm nữa");
                                    }
                                    break;
                            }
                        }
        ...
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    ...
                    if (this.mapId == 5) {
                        this.createOtherMenu(player, 0,
                                "\b|2|Con muốn gì nào?\nCon đang còn : " + player.pointPvp + " \b|7|điểm Mua Sắm", "Đến Siêu Thị 2.0", "Đổi Đồ", "Top Mua Sắm");
                    }
                    if (this.mapId == 129) {
                        this.createOtherMenu(player, 0,
                                "Con muốn gì nào?", "Quay ve");
...
//                                        
                            ...
                            case 2:
                                if (player.getSession().player.nPoint.power == 1) {
                                    ...
                                    this.npcChat(player, "Chức năng này đang được cập nhật");
                                }
                                break;
                        }
                    }
                        ...
                        if (player.iDMark.isBaseMenu()) {
                            ...
                            switch (select) {
                                case 0:
                                    ...
                                    Service.gI().sendThongBao(player, "Cu cậu bị bế đi rồi =((...");
                                    break;
                                case 1:
                                    ChangeMapService.gI().changeMapBySpaceShip(player, 109, -1, 295);
                                    break;
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    createOtherMenu(player, ConstNpc.BASE_MENU,
                            "HELLO friend. tôi mới trộm của Shin được ít đồ cậu có muốn xem không?",
                            "Cửa hàng", "Shin no love", "Quy đổi", "Shop Vip");
                }
            }
    ...
    public static Npc thodaika(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    createOtherMenu(player, ConstNpc.BASE_MENU,
                            "Xã hội này chỉ có làm thì mới có ăn, cần cù thì bù siêng năng. Nên là \"Tài Xỉu đi anh em \" =))",
                            "Xỉu", "Tài");
                }
            }
...
//                                                CombineService.gI().openTabCombine(player, CombineService.EP_SAO_TRANG_BI);
                    ...
                    } else if (this.mapId == 42 || this.mapId == 43 || this.mapId == 44 || this.mapId == 84 || this.mapId == 176) {
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                case 0: //shop bùa
    ...
    public static Npc duongtank(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (mapId == 0) {
                        this.createOtherMenu(player, 0, "Ngũ Hàng Sơn x2 Tnsm\nHỗ trợ cho Ae Từ\b|1|Dưới 1tr5 SM dến 80 Tỷ SM?", "OK", "hong be oi");
                    }
                    if (mapId == 123) {
                        this.createOtherMenu(player, 0, "Bạn Muốn Quay Trở Lại Làng Ảru?", "OK", "Từ chối");
                    ...
                    if (mapId == 122) {
                        this.createOtherMenu(player, 0, " Thiện style thiện style\b|7|Thí chủ đang có: " + player.NguHanhSonPoint + " điểm ngũ hành sơn\b|1|Thí chủ muốn đổi cải trang x4 chưởng ko?", "Âu kê", "Top Ngu Hanh Son", "No");
                    }
                }
            }
            ...
            public void confirmMenu(Player player, int select) {
                if (canOpenNpc(player)) {
                    switch (select) {
                        case 0:
                            if (mapId == 0) {
                                if (player.nPoint.power < 1500000 || player.nPoint.power >= 800000000000L) {
                                    Service.gI().sendThongBao(player, "Bạn tuổi l qua đuọc  map!");
                                    return;
                                }
        ...
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (this.mapId == 46) {
                        if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "|1|Trả lời câu hỏi sau: Shin có đẹp trai khum?"
                                    + "\n|2| Shin"
                                    + "\n|3|Không"
                                    + "\n|3|"
                                    + "\n|4|Péo"
                        ...
                        this.createOtherMenu(player, ConstNpc.BASE_MENU, "Hello my friend. friend muốn cái gì?!", "Cửa hàng", "Đóng");
                    }
                }
            }
            ...
            public void confirmMenu(Player player, int select) {
                if (canOpenNpc(player)) {
                    if (this.mapId == 46) {
                        if (player.iDMark.isBaseMenu()) {
                            if (select == 0) {
                                ShopServiceNew.gI().opendShop(player, "!", true);
                            }
                        }
                    } else if (this.mapId == 104) {
                        if (player.iDMark.isBaseMenu()) {
    ...
    public static Npc vados(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    createOtherMenu(player, ConstNpc.BASE_MENU,
                            "|2|em vừa mua được hack map\b|7|Senpai Muốn Xem Tóp Gì?",
    ...
    public static Npc khidaumoi(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (this.mapId == 14) {
                    this.createOtherMenu(player, ConstNpc.BASE_MENU,
                            "Bạn muốn gì ở tôi?",  "Shop của Khỉ", "Từ chối");
                }
            }
